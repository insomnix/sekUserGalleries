...
{% for (var i=0, files=o.files, l=files.length, file=files[0]; i<l; file=files[++i]) { %}
    <tr class="template-upload fade">
        <td class="preview"><span class="fade"></span><br />{%=file.name%}<br />{%=o.formatFileSize(file.size)%}</td>
        <td class="name">
        ...
        {% if (file.error) { %}
            <td class="error" colspan="2"><span class="label label-important">[[%sekug.error]]</span> {%=fileUploadErrors[file.error] || file.error%}</td>
