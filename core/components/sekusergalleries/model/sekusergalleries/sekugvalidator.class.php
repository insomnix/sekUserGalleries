 ...
 * User Galleries
 *
 * sekugValidator based on LoginValidator by Shaun McCormick
...
class sekugValidator {
    ...
    public $modx = null;
    /**
     * @var sekUserGalleries $sekug A reference to the UserGalleries instance.
     * @access public
     */
    public $sekug = null;

    /**
     * The constructor for the sekugValidator class
     *
     * @param sekUserGalleries &$sekug A reference to the sekUserGalleries class instance.
     * @param array $config Optional. An array of configuration parameters.
     * @return sekugValidator
     */
    function __construct(sekUserGalleries &$sekug,array $config = array()) {
        ...
        $this->config = array_merge(array(
            ...
            'customValidators' => !empty($this->sekug->config['customValidators']) ? explode(',',$this->sekug->config['customValidators']) : array(),
        ),$config);
    }
     ...
     *
     * @access public
     * @param sekUserGalleriesDictionary $dictionary The fields to validate.
     * @param string $validationFields
     * @return array An array of field name => value pairs.
     */
    public function validateFields(sekugDictionary $dictionary,$validationFields = '') {
    ...
    public function validate($key,$value,$type = '') {
        ...
        /* only allow specified validators to prevent brute force execution of unwanted snippets */
            ...
            if ($snippet) {
                ...
                $props = array_merge($this->sekug->config,array(
                    'key' => $key,
                    'value' => $value,
                    'param' => $param,
                    'type' => $type,
                ...
                /* no validator found */
                $this->modx->log(modX::LOG_LEVEL_ERROR,'[sekUserGalleries] Could not find validator "'.$type.'" for field "'.$key.'".');
                $validated = true;
            }
        } else {
            $this->modx->log(modX::LOG_LEVEL_INFO,'[sekUserGalleries] Validator "'.$type.'" for field "'.$key.'" was not specified in the customValidators property.');
            $validated = true;
        }
    ...
    public function addError($key,$value) {
        $errTpl = $this->modx->getOption('errTpl',$this->sekug->config,'<span class="error">[[+error]]</span>');
    ...
    protected function _getErrorMessage($field,$parameter,$lexiconKey,array $properties = array()) {
        if (!empty($this->sekug->config[$field.'.'.$parameter])) {
            $message = $this->sekug->config[$field.'.'.$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else if (!empty($this->sekug->config[$parameter])) {
            $message = $this->sekug->config[$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else {
